...
if fetch_clicked:
    if not ticker.strip():
        ...
        if data is None:
            symbol_display = ticker.upper() + (".LG" if exchange == "NGX" else "")
            st.sidebar.warning(
                f"⚠️ Could not fetch data for **{symbol_display}**.\n\n"
                "Possible reasons:\n"
                "- Ticker misspelled or not on Yahoo Finance\n"
                "- NGX coverage is partial for this stock\n"
                "- yfinance API temporarily unavailable\n\n"
                "Try checking spelling, or fill values in manually below."
            )
            st.session_state.fetched_data = None
